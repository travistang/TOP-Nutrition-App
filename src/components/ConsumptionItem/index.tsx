...
export default function ConsumptionItem({ mealCalories, consumption, onClick }:Props) {
  const { targetCalories } = useRecoilValue(dailyNutritionGoalAtom);
  const itemCalories = NutritonUtils.caloriesByAmount(consumption.nutritionPerHundred, consumption.amount);
  ...
  return (
    <div className="flex-shrink-0 grid grid-cols-6 items-center w-full h-14 py-2" onClick={onClick}>
      ...
      <div className="col-span-2 flex flex-col items-end">
        <span className="font-bold text-sm text-right">{itemCalories.toFixed(1)} kcal</span>
        <span className="text-xs text-yellow-700">{(calorieRatioAgainstMeal * 100).toFixed(1)}% of meal</span>
      </div>
